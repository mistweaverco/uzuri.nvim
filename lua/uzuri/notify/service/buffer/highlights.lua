...
---@class UzuriNotifyBufHighlights
---@field groups table
...
---@field buffer number
---@field _config table
local UzuriNotifyBufHighlights = {}

function UzuriNotifyBufHighlights:new(level, buffer, config)
  local function linked_group(section)
    local orig = "UzuriNotify" .. level .. section
    if vim.fn.hlID(orig) == 0 then
      orig = "UzuriNotifyINFO" .. section
    end
...
function UzuriNotifyBufHighlights:_redefine_treesitter()
  local buf_highlighter = require("vim.treesitter.highlighter").active[self.buffer]

  if not buf_highlighter then
    return
  end
  local render_namespace = vim.api.nvim_create_namespace("uzuri.notify.treesitter.override")
...
function UzuriNotifyBufHighlights:set_opacity(alpha)
  if
    not self._treesitter_redefined and vim.api.nvim_get_option_value("filetype", { buf = self.buffer }) ~= "notify"
  then
...
function UzuriNotifyBufHighlights:get_opacity()
  return self.opacity
end

---@return UzuriNotifyBufHighlights
return function(level, buffer, config)
  return UzuriNotifyBufHighlights:new(level, buffer, config)
end
