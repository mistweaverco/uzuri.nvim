local api = vim.api
local animate = require("uzuri.notify.animate")
...
---@field notif_bufs table<number, UzuriNotificationBuf>
---@field timers table
---@field stages table
  ...
  for field, goal in pairs(goals) do
    if field ~= "time" then
      ...
      elseif goal_type ~= "table" then
        ...
        error("uzuri.notify: Invalid stage goal: " .. vim.inspect(goal))
      end
    end
  end
