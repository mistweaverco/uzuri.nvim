local global_config = require("uzuri.config")
local map_util = require("uzuri.map_util")
local patch = require("uzuri.patch")
local util = require("uzuri.util")
local M = {}

---@alias uzuri.Mode "insert" | "visual" | "normal" | "select"

---@class (exact) uzuri.InputContext
---@field opts? uzuri.InputOptions
---@field on_confirm? fun(text?: string)
---@field winid? integer
---@field history_idx? integer
---@field history_tip? string
---@field mode_to_restore? uzuri.Mode The mode to restore when the input window closes.

---@class (exact) uzuri.InputConfig
---@field start_in_insert? boolean
---@field start_mode? uzuri.Mode
...
---@class (exact) uzuri.InputOptions
---@field prompt? string
...
local keymaps = {
  ...
  },
  {
    ...
    plug = "<Plug>UzuriInput:HistoryPrev",
    rhs = function()
      M.history_prev()
    end,
  },
  {
    ...
    plug = "<Plug>UzuriInput:HistoryNext",
    rhs = function()
      M.history_next()
    end,
  },
...
local function apply_highlight()
  ...
  local ns = vim.api.nvim_create_namespace("UzuriHighlight")
  local highlights = {}
...
---@param opts string|uzuri.InputOptions
...
local show_input = util.make_queued_async_fn(2, function(opts, on_confirm)
  ...
  local config = global_config.get_mod_config("input", opts) --[[@as uzuri.InputConfig]]
  ...
  for mode, user_maps in pairs(config.mappings) do
    map_util.create_maps_to_plug(bufnr, mode, user_maps, "UzuriInput:")
  end

  vim.bo[bufnr].filetype = "UzuriInput"
  ...
  if opts.completion then
    vim.bo[bufnr].completefunc = "v:lua.require'uzuri.input'.completefunc"
    vim.bo[bufnr].omnifunc = "v:lua.require'uzuri.input'.completefunc"
