...
local function clear_callback()
  _G.uzuri_fzf_choice = function() end
  _G.uzuri_fzf_cancel = function() end
end
...
M.select = function(config, items, opts, on_choice)
  ...
  _G.uzuri_fzf_cancel = function()
    clear_callback()
    on_choice(nil, nil)
  end
  _G.uzuri_fzf_choice = function(label)
    clear_callback()
  ...
  vim.fn["uzuri#fzf_run"](labels, string.format('--prompt="%s "', opts.prompt), config.window)
  -- fzf doesn't have a cancel callback, so we have to make one.
  vim.cmd([[autocmd TermClose <buffer> ++once lua require('uzuri.select.fzf')._on_term_close()]])
end
